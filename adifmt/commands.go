...
var (
	...
	countConf = cmdConfig{Command: cmd.Count,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.StringVar(&cctx.CountFieldName, "count-field", "APP_ADIFMT_COUNT",
				"Field `name` for record counts")
			fs.Var(&cctx.Fields, "fields",
				"Comma-separated or multiple instance field `names` to group by")
			ctx.CommandCtx = &cctx
		},
	}
	...
	editConf = cmdConfig{Command: cmd.Edit,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.Var(cctx.Cond.IfFlag(), "if",
				"Only edit records where `condition` is true (repeatable)")
			fs.Var(cctx.Cond.IfNotFlag(), "if-not",
				"Only edit records where `condition` is false (repeatable)")
			fs.Var(cctx.Cond.OrIfFlag(), "or-if",
				"Only edit records where `condition` is true or any previous --if group\nmatches (repeatable)")
			fs.Var(cctx.Cond.OrIfNotFlag(), "or-if-not",
				"Only edit records where `condition` is false or any previous --if group\nmatches (repeatable)")
			fs.Var(&cctx.Add, "add",
				"Add `field=value` if field is not already in a record (repeatable)")
			fs.Var(&cctx.Set, "set",
				"Set `field=value` for all records (repeatable)")
			fs.Var(&cctx.Rename, "rename",
				"Rename `old=new` field for all records (repeatable)")
			fs.Var(&cctx.Remove, "remove",
				"Remove `fields` from all records (comma-separated, repeatable)")
			fs.BoolVar(&cctx.RemoveBlank, "remove-blank", false,
				"Remove all blank fields")
			fs.Var(&cctx.FromZone, "time-zone-from",
				"Adjust times and dates from this time `zone` into --time-zone-to\n(default UTC)")
			fs.Var(&cctx.ToZone, "time-zone-to",
				"Adjust times and dates into this time `zone` from --time-zone-from\n(default UTC)")
			ctx.CommandCtx = &cctx
		}}
	...
	findConf = cmdConfig{Command: cmd.Find,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.Var(cctx.Cond.IfFlag(), "if",
				"Include records where `condition` is true (repeatable)")
			fs.Var(cctx.Cond.IfNotFlag(), "if-not",
				"Include records where `condition` is false (repeatable)")
			fs.Var(cctx.Cond.OrIfFlag(), "or-if",
				"Include records where `condition` is true or any previous --if group\nmatches (repeatable)")
			fs.Var(cctx.Cond.OrIfNotFlag(), "or-if-not",
				"Include records where `condition` is false or any previous --if group\nmatches (repeatable)")
			ctx.CommandCtx = &cctx
		}}
	...
	flattenConf = cmdConfig{Command: cmd.Flatten,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.Var(&cctx.Delimiters, "delimiter",
				"`field=delim` to split field around character sequence delim (repeatable)\nOnly needed if delim isn't implied by field's type")
			fs.Var(&cctx.Fields, "fields",
				"Comma-separated or multiple instance field `names` to flatten")
			ctx.CommandCtx = &cctx
		}}
	...
	helpConf = cmdConfig{Command: cmd.Command{
		...
		Run: func(*cmd.Context, []string) error {
			return nil // handled specially by main
		}}}
	...
	saveConf = cmdConfig{Command: cmd.Save,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.BoolVar(&cctx.CreateDirectory, "create-dirs", false,
	...
	selectConf = cmdConfig{Command: cmd.Select,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.Var(&cctx.Fields, "fields",
				"Comma-separated or multiple instance field `names` to include in output")
			ctx.CommandCtx = &cctx
		}}
	...
	sortConf = cmdConfig{Command: cmd.Sort,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.Var(&cctx.Fields, "fields",
				"Comma-separated or multiple instance field `names` to sort by")
			ctx.CommandCtx = &cctx
		}}
	...
	validateConf = cmdConfig{Command: cmd.Validate,
		Configure: func(ctx *cmd.Context, fs *flag.FlagSet) {
			...
			fs.Var(cctx.Cond.IfFlag(), "if",
				"Only check required-fields when `condition` is true (repeatable)")
			fs.Var(cctx.Cond.IfNotFlag(), "if-not",
				"Only check required-fields when `condition` is false (repeatable)")
			fs.Var(cctx.Cond.OrIfFlag(), "or-if",
				"Only check required-fields when `condition` is true or any\nprevious --if group matches (repeatable)")
			fs.Var(cctx.Cond.OrIfNotFlag(), "or-if-not",
				"Only check required-fields when `condition` is false or any\nprevious --if group matches (repeatable)")
			fs.Var(&cctx.RequiredFields, "required-fields",
				"Field `names` which must be present and non-empty in a valid record")
			ctx.CommandCtx = &cctx
		}}
