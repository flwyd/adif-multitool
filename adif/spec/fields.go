...
var (
	...
	// ProgramidField is a String, identifies the name of the logger, converter, or utility that created or processed this ADIF file To help avoid name clashes, the ADIF PROGRAMID Register provides a voluntary list of PROGRAMID values.
	...
	// CntyAltField is a SecondaryAdministrativeSubdivisionListAlt, a semicolon (;) delimited, unordered list of Secondary Administrative Subdivision Alt codes for the contacted station See the Data Type for details.
	CntyAltField = Field{
		Name: "CNTY_ALT",
		Type: SecondaryAdministrativeSubdivisionListAltDataType,
	}
	// CommentField is a String, comment field for QSO for a message to be incorporated in a paper or electronic QSL for the contacted station's operator, use the QSLMSG field recommended use: information of interest to the contacted station's operator.
	CommentField = Field{
		Name: "COMMENT",
		Type: StringDataType,
	}
	// CommentIntlField is a IntlString, comment field for QSO for a message to be incorporated in a paper or electronic QSL for the contacted station's operator, use the QSLMSG_INTL field recommended use: information of interest to the contacted station's operator.
	...
	// DclQslrdateField is a Date, date QSL received from DCL (only valid if DCL_QSL_RCVD is Y, I, or V)(V import-only).
	DclQslrdateField = Field{
		Name: "DCL_QSLRDATE",
		Type: DateDataType,
	}
	// DclQslsdateField is a Date, date QSL sent to DCL (only valid if DCL_QSL_SENT is Y, Q, or I).
	DclQslsdateField = Field{
		Name: "DCL_QSLSDATE",
		Type: DateDataType,
	}
	// DclQslRcvdField is a Enumeration, DCL QSL received status Default Value: N.
	DclQslRcvdField = Field{
		Name:     "DCL_QSL_RCVD",
		Type:     EnumerationDataType,
		EnumName: "QSL_Rcvd",
	}
	// DclQslSentField is a Enumeration, DCL QSL sent status Default Value: N.
	DclQslSentField = Field{
		Name:     "DCL_QSL_SENT",
		Type:     EnumerationDataType,
		EnumName: "QSL_Sent",
	}
	...
	// EqslAgField is a Enumeration, indicates whether the QSO is known to be "Authenticity Guaranteed" by eQSL.
	EqslAgField = Field{
		Name:     "EQSL_AG",
		Type:     EnumerationDataType,
		EnumName: "EQSL_AG",
	...
	// MorseKeyInfoField is a String, details of the contacted station's Morse key (e.g. make, model, etc). Example: <MORSE_KEY_INFO:16>Begali Sculpture.
	MorseKeyInfoField = Field{
		Name: "MORSE_KEY_INFO",
		Type: StringDataType,
	}
	// MorseKeyTypeField is a Enumeration, the contacted station's Morse key type (e.g. straight key, bug, etc). Example for a dual-lever paddle: <MORSE_KEY_TYPE:2>DP.
	MorseKeyTypeField = Field{
		Name:     "MORSE_KEY_TYPE",
		Type:     EnumerationDataType,
		EnumName: "Morse_Key_Type",
	}
	...
	// MyCntyAltField is a SecondaryAdministrativeSubdivisionListAlt, a semicolon (;) delimited, unordered list of Secondary Administrative Subdivision Alt codes for the logging station See the Data Type for details.
	MyCntyAltField = Field{
		Name: "MY_CNTY_ALT",
		Type: SecondaryAdministrativeSubdivisionListAltDataType,
	}
	...
	// MyDarcDokField is a Enumeration, the logging station's DARC DOK (District Location Code) A DOK comprises letters and numbers, e.g. <MY_DARC_DOK:3>A01.
	MyDarcDokField = Field{
		Name: "MY_DARC_DOK",
		Type: EnumerationDataType,
	}
	...
	// MyMorseKeyInfoField is a String, details of the logging station's Morse key (e.g. make, model, etc). Example: <MY_MORSE_KEY_INFO:16>Begali Sculpture.
	MyMorseKeyInfoField = Field{
		Name: "MY_MORSE_KEY_INFO",
		Type: StringDataType,
	}
	// MyMorseKeyTypeField is a Enumeration, the logging station's Morse key type (e.g. straight key, bug, etc). Example for a dual-lever paddle: <MORSE_KEY_TYPE:2>DP.
	MyMorseKeyTypeField = Field{
		Name:     "MY_MORSE_KEY_TYPE",
		Type:     EnumerationDataType,
		EnumName: "Morse_Key_Type",
	}
	...
	// MyVuccGridsField is a GridSquareList, two or four adjacent Maidenhead grid locators, each four or six characters long, representing the logging station's grid squares that the contacted station may claim for the ARRL VUCC award program. E.g. EM98,FM08,EM97,FM07.
	...
	// QrzcomQsoDownloadDateField is a Date, date QSO downloaded from QRZ.COM logbook.
	QrzcomQsoDownloadDateField = Field{
		Name: "QRZCOM_QSO_DOWNLOAD_DATE",
		Type: DateDataType,
	}
	// QrzcomQsoDownloadStatusField is a Enumeration, QRZ.COM logbook QSO download status.
	QrzcomQsoDownloadStatusField = Field{
		Name:     "QRZCOM_QSO_DOWNLOAD_STATUS",
		Type:     EnumerationDataType,
		EnumName: "QSO_Download_Status",
	}
	...
	// QslmsgField is a MultilineString, a message for the contacted station's operator to be incorporated in a paper or electronic QSL.
	QslmsgField = Field{
		Name: "QSLMSG",
		Type: MultilineStringDataType,
	}
	// QslmsgIntlField is a IntlMultilineString, a message for the contacted station's operator to be incorporated in a paper or electronic QSL.
	QslmsgIntlField = Field{
		Name: "QSLMSG_INTL",
		Type: IntlMultilineStringDataType,
	}
	// QslmsgRcvdField is a MultilineString, a message addressed to the logging station's operator incorporated in a paper or electronic QSL.
	QslmsgRcvdField = Field{
		Name: "QSLMSG_RCVD",
		Type: MultilineStringDataType,
	}
	...
	// VuccGridsField is a GridSquareList, two or four adjacent Maidenhead grid locators, each four or six characters long, representing the contacted station's grid squares credited to the QSO for the ARRL VUCC award program. E.g. EM98,FM08,EM97,FM07.
...
func init() {
	...
	Fields["DCL_QSLRDATE"] = DclQslrdateField
	Fields["DCL_QSLSDATE"] = DclQslsdateField
	Fields["DCL_QSL_RCVD"] = DclQslRcvdField
	Fields["DCL_QSL_SENT"] = DclQslSentField
	Fields["DISTANCE"] = DistanceField
	Fields["DXCC"] = DxccField
	Fields["EMAIL"] = EmailField
	Fields["EQ_CALL"] = EqCallField
	Fields["EQSL_AG"] = EqslAgField
	...
	Fields["MORSE_KEY_INFO"] = MorseKeyInfoField
	Fields["MORSE_KEY_TYPE"] = MorseKeyTypeField
	...
	Fields["MY_MORSE_KEY_INFO"] = MyMorseKeyInfoField
	Fields["MY_MORSE_KEY_TYPE"] = MyMorseKeyTypeField
	...
	Fields["QRZCOM_QSO_DOWNLOAD_DATE"] = QrzcomQsoDownloadDateField
	Fields["QRZCOM_QSO_DOWNLOAD_STATUS"] = QrzcomQsoDownloadStatusField
	...
	Fields["QSLMSG_RCVD"] = QslmsgRcvdField
